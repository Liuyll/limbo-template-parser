...
function parse(source) {
    ...
    const readTag = () => {
        ...
        while (true) {
            ...
            if (char === 'l' && peep(1) === '-') {
                ...
                if (maybeDirectiveStr in innerDirectiveLists) {
                    ...
                    if (maybeDirectiveStr === 'l-else' || maybeDirectiveStr === 'l-elif') {
                        if (!unEndCondition)
                            error_1.errors(error_1.Errors.UnexpectedConditionDirective, { condition: maybeDirectiveStr, line, column });
                        existCondition = true;
                    }
            ...
            // eg: l-key="value"
                ...
                error_1.errors(error_1.Errors.UnexpectedChar, { column, line, char });
            }
            else if (char === '}') {
                error_1.errors(error_1.Errors.UnexpectedChar, { column, line, char });
            }
    ...
    const walk = (parent) => {
        ...
        while (index < source.length) {
            ...
            if (char === '}') {
                if (!binding)
                    error_1.errors(error_1.Errors.UnexpectedRightBrace, { line, column });
                binding--;
                if (!binding)
                    flushText();
                else
